...
// мир замкнут в кольцо (21.09.2026, бесшовно): x всегда в [0, WORLD_W), край z6 стыкуется с
// началом z0. wrapX — привести координату в этот диапазон; wrapDelta — кратчайшая ЗНАКОВАЯ
// разница b-a по кольцу (может быть отрицательной — используется для направления движения);
// wrapDist — её модуль. Работает одинаково корректно и для комнат (там x на порядки меньше
// WORLD_W, поправка на стык никогда не срабатывает) — поэтому применяем везде без разбора.
const wrapX     = x => ((x % WORLD_W) + WORLD_W) % WORLD_W;
const wrapDelta = (a, b) => { let d = (b - a) % WORLD_W; if (d > WORLD_W / 2) d -= WORLD_W; else if (d < -WORLD_W / 2) d += WORLD_W; return d; };
const wrapDist  = (a, b) => Math.abs(wrapDelta(a, b));
const zoneAt = x => Math.min(N_ZONES - 1, Math.floor(wrapX(x) / ZONE_W));
...
// ── ГАРАЖ: вход на улице (зона 2, по центру) — комната с готовой картинкой-фоном (не тайлы, как
// в клубе). Игроки видят только тех, кто внутри, драться нельзя, ботов нет — тот же принцип, что в клубе.
// Дверь — ровно у тайла "открытый гараж со светящейся машиной" (garageCar) на фасаде района, а не
// просто по центру зоны — тайл выбран тот, что ближе к площади (второй по счёту в повторе узора).
const GARAGE_DOOR_X = 2 * ZONE_W + 1500;
const GARAGE = {
  x0: 40, x1: 1240, y0: 380, y1: 700, spawn: { x: 640, y: 640 }, exit: { x0: 560, x1: 760, y: 680 },
  blocks: [ { x0: 0, x1: 660, y0: 380, y1: 410 }, { x0: 930, x1: 1280, y0: 380, y1: 415 } ],
};
function garageConstrain(e) {
  e.x = clamp(e.x, GARAGE.x0, GARAGE.x1); e.y = clamp(e.y, GARAGE.y0, GARAGE.y1);
  for (const b of GARAGE.blocks) {
...
// ── ПИЦЦЕРИЯ: вход на улице (зона 4, у ВТОРОГО тайла "pizza", дальше от площади) — точно на
// нарисованной двери слева от вывески (не по центру тайла, дверь смещена к левому краю
...
// Границы пола — по красной линии, которую нарисовал пользователь поверх референса (найдены
// анализом пикселей его разметки): плавный откос у левой кабинки, ровная середина у барной
// стойки, откос и глубокая "мёртвая зона" у столиков/холодильника справа.
const PIZZERIA = {
  x0: 40, x1: 1240, y0: 371, y1: 700, spawn: { x: 640, y: 600 }, exit: { x0: 560, x1: 760, y: 680 },
  blocks: [
    { x0: 0, x1: 60, y0: 371, y1: 429 }, { x0: 60, x1: 120, y0: 371, y1: 417 },
    { x0: 120, x1: 180, y0: 371, y1: 403 }, { x0: 180, x1: 240, y0: 371, y1: 388 },
    { x0: 240, x1: 300, y0: 371, y1: 379 },
    { x0: 840, x1: 900, y0: 371, y1: 377 }, { x0: 900, x1: 960, y0: 371, y1: 385 },
    { x0: 960, x1: 1020, y0: 371, y1: 399 }, { x0: 1020, x1: 1080, y0: 371, y1: 414 },
    { x0: 1080, x1: 1140, y0: 371, y1: 426 }, { x0: 1140, x1: 1200, y0: 371, y1: 432 },
    { x0: 1200, x1: 1240, y0: 371, y1: 615 },
  ],
};
function pizzeriaConstrain(e) {
  // блоки здесь — не отдельные препятствия (как в гараже), а локальное поднятие границы пола
  // в конкретной колонке x (лестница по нарисованной пользователем линии), поэтому просто
  // не пускаем игрока выше b.y1 в этом x-диапазоне — без общего "вытолкнуть к ближайшему краю".
  e.x = clamp(e.x, PIZZERIA.x0, PIZZERIA.x1); e.y = clamp(e.y, PIZZERIA.y0, PIZZERIA.y1);
  for (const b of PIZZERIA.blocks) {
    if (e.x > b.x0 && e.x < b.x1 && e.y < b.y1) e.y = b.y1;
  }
}
...
// ×1.6 — реальная скорость доходила до ~540 пикс/с, «летали». Затем весь диапазон домножен на
// 0.75, а боты переведены на run=1 (было 0.75, см. spawnBot) — так свежий игрок (spd 4-7 в
// зависимости от бойца) двигается вровень с ботом (spd 4) с точностью до пары пикселей: обоим
// нужна одна и та же формула с одним и тем же run — по прямой просьбе «игрок изначально должен
// идти как бот».
...
function spawnBot(zone) {
  ...
  b.hp = maxHpOf(b); b.run = 1; b.wait = rnd(0, 3);   // run=1 — та же база, что у игрока (см. speedOf)
  ents.set(b.id, b);
}
...
function canDamage(att, tgt) {
  ...
  if (att.room !== tgt.room) return false;   // через комнаты (и с улицей) не бьют
  ...
  if (att.room) return true;   // в комнатах (клуб/гараж/пиццерия и любые новые) драться можно всегда, как в «диком» районе
...
function resolveAttack(att) {
  ...
  for (const t of ents.values()) {
    ...
    const ddx = wrapDelta(att.x, t.x) * att.f;
...
async function clanInvite(p, toId) {
  ...
  if (!t || t.kind !== 'p' || t === p || t.room !== p.room || wrapDist(p.x, t.x) > 900) return;
...
function botAI(b, dt) {
  ...
  if (b.boss) { bossAI(b, dt); return; }
  let tgt = b.target ? ents.get(b.target) : null;
  if (tgt && (tgt.koT > 0 || tgt.room || zoneAt(tgt.x) !== b.zone || wrapDist(b.x, tgt.x) > 700)) { tgt = null; b.target = 0; }
  if (!tgt) {
    let best = 1e9;
    for (const p of ents.values()) {
      if (p.kind !== 'p' || p.koT > 0 || p.room || zoneAt(p.x) !== b.zone) continue;
      const dx = wrapDist(b.x, p.x), dy = Math.abs(p.y - b.y);
      if (dx < 260 && dy < 140 && dx < best) { best = dx; tgt = p; }
    }
    if (tgt) b.target = tgt.id;
  }
  if (tgt) {
    const dx = wrapDelta(b.x, tgt.x), dy = tgt.y - b.y;
  ...
  const dx = wrapDelta(b.x, b.tx), dy = b.ty - b.y, d = Math.hypot(dx, dy);
  if (d < 8) {
    b.wait = rnd(0.8, 3);
...
function bossAI(b, dt) {
  ...
  if (!tgt) {
    let best = 1e9;
    for (const p of ents.values()) {
      if (p.kind !== 'p' || p.koT > 0 || p.room !== b.room) continue;
  ...
  if (tgt) {
    ...
    if (Math.abs(dx) > 95 || Math.abs(dy) > 28) { const d = Math.hypot(dx, dy) || 1; b.inx = dx / d; b.iny = dy / d; }
      ...
      b.inx = b.iny = 0;
      if (b.cd <= 0 && startAttack(b, 'punch')) b.cd = rnd(1.1, 1.8);
    }
    return;
  }
  if (b.wait > 0) { b.wait -= dt; b.inx = b.iny = 0; return; }
...
function tick() {
  ...
  for (const e of ents.values()) {
    ...
    else { e.x = wrapX(e.x + dx * sp * dt); e.y = clamp(e.y + dy * sp * 0.6 * dt, GROUND_MIN, GROUND_MAX); }   // открытый мир — кольцо, без краёв
...
function broadcast() {
  ...
  for (const p of byAccount.values()) {
    ...
    for (const e of list) {
      if (e.room !== p.room || (!p.room && wrapDist(p.x, e.x) > VIEW_RANGE)) continue;   // видим только свою комнату
...
function enterClub(p) {
  ...
  if (wrapDist(p.x, CLUB_DOOR_X) > 140 || p.y > 570) return;          // только вплотную к двери
...
function enterGarage(p) {
  if (p.room || p.koT > 0) return;
  if (wrapDist(p.x, GARAGE_DOOR_X) > 140 || p.y > 570) return;
  if (p.pvpT < CLUB_COMBAT_LOCK) { if (p.hintCd <= 0) { p.hintCd = 2; toast(p, 'no_enter_combat', '#ffc933'); } return; }
  p.room = 'garage'; p.x = GARAGE.spawn.x; p.y = GARAGE.spawn.y; p.inx = p.iny = 0; p.atk = null; p.blk = false; p.hurtT = 0;
}
function leaveGarage(p) {
...
function enterPizzeria(p) {
  if (p.room || p.koT > 0) return;
  if (wrapDist(p.x, PIZZERIA_DOOR_X) > 140 || p.y > 570) return;
  if (p.pvpT < CLUB_COMBAT_LOCK) { if (p.hintCd <= 0) { p.hintCd = 2; toast(p, 'no_enter_combat', '#ffc933'); } return; }
