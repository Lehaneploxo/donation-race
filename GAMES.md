...
| 🔥 Разнос | `/razgon` | `client/razgon.html` | 11-я игра (01.09.2026, вдохновлено роаст-форматом с говорящей головой на TikTok LIVE). НЕ игра в привычном смысле — оверлей поверх LIVE, тот же движок говорящего ведущего (Live2D "Kei"), что в «Взаимках» (`client/sprites/kei_live2d/`). Зритель подписывается + лайкает + пишет в чат «разнеси» → попадает в очередь → ведущий зачитывает шутливую роаст-фразу с его ником (озвучка browser TTS). Подписка/лайк засчитываются только из реального эфира (демо-боты не наполняют очередь — та же защита, что у `vzaimki_follow`/`vzaimki_like`). После разноса — 45 сек кулдаун, прежде чем зритель снова может встать в очередь. Все данные (кто подписался/лайкнул/в очереди) только в памяти текущего эфира, сбрасываются на каждый новый LIVE. Донат-механики (пропуск очереди и т.п., как в игре-референсе) сознательно не сделаны в первой версии — только базовый цикл. Отладка: `?debug=1`|
